function [PrecisionT, RecallT, F_mT, errorT, PrecisionTR, RecallTR, F_mTR, errorTR] = npmpgm_em_main(dataTrainRaw,dataTest,row_map,col_map,epohs_map,val_dirichlet)
...
%% ќбучение модели
if USETRAIN == 1    
   [Probability, cellNetKox] = npmpgm_em_train(dataTrainRaw,dataTrainForClass, ...
...
%% test on test data
[PrecisionT, RecallT, F_mT, errorT] = npmpgm_em_test_l(dataTest,Probability,cellNetKox);
%% test on train data
[PrecisionTR, RecallTR, F_mTR, errorTR] = npmpgm_em_test_l(dataTrainRaw,Probability,cellNetKox);

function [AveragePrecision, AverageRecall, F_measure, error] = npmpgm_em_test_l(dataTest,Probability,cellNetKox)
k = 1;
...
dataTest = dataTest(:);
[arrayLL] = npmpgm_em_test(Probability,cellNetKox,dataTest);
