function [R] = arabic_digit_hcrf_train(R,dataTrainArabicDigit,NUMBER)
%% about
...
for i=1:size(dataTrainArabicDigitKox,1)
    for j=1:size(dataTrainArabicDigitKox,2)
        dataTrain(1,index) = dataTrainArabicDigitKox(i,j);
        labelTrain(1,index) = labelTrainArabicDigit(i,j);
        index = index +1;
    end;
end;
dataTrainArabicDigitKox = dataTrain;
labelTrainArabicDigit = labelTrain;
