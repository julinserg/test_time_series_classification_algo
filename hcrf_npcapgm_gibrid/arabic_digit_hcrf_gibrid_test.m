function [loglikeTestArabicDigit labelTestArabicDigit] = arabic_digit_hcrf_gibrid_test(dataTestArabicDigit,NUMBER)
%% about
%   классификация арабских цифр с помощью HCRF laybrary - тестирование

%% load test data
load arrayLogLikDataSetTest
fprintf('..........Start test\n');
ENABLE_KOH = 0;
...
for i=1:size(dataTestArabicDigit,1)
    for j=1:size(dataTestArabicDigit,2)
        ...
        s = size(dataTestArabicDigit{i,j},1);
        s1 = size(arrayLogLikDataSetTest{i,j},1);
        dataTestArabicDigit{i,j}(s+1:s+s1,:) = arrayLogLikDataSetTest{i,j}(:,:);
    end;
end;
